...
def main():
    ...
    print(now(), '--其他站开始签到--')
    [signin_discuz_dsu(config['session'], config['url']) for config in generateConfig() if 'sign_in_discuz' in config['tasks']]
    [signin_hifi(config['session'], config['url']) for config in generateConfig() if 'sign_in_hifi' in config['tasks']]
    # cookie过期发送qq推送信息
    r = re.compile(r'过期')
    if r.search(txt):
        sendQmsgInfo(txt)
